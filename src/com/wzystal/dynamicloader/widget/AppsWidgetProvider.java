...
public class AppsWidgetProvider extends AppWidgetProvider {
	private static final String CLASS_NAME = "AppsWidgetProvider";
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
		...
		for (int widgetId : appWidgetIds) {
			...
			// AppsWidgetService继承自RemoteViewsService，用于管理复杂视图（类似Adapter）
			Intent serviceIntent = new Intent(context, AppsWidgetService.class);
			// RemoteViews将复杂视图与其对应的Adapter绑定在一起
			remoteViews.setRemoteAdapter(R.id.gridview_apps, serviceIntent);
			// 为复杂视图中的item设置响应事件绑定
			Intent itemIntent = new Intent();
			itemIntent.setAction(ACTION_ITEM_CLICK);
